...
public class MapView extends ImageView {
    ...
    public MapView(Context context, AttributeSet attrs) {
        ...
        rooms = connection.raumGet();
        roomEnterance = connection.raumEingangGet();
    ...
    protected void onDraw(Canvas canvas) {
       ...
       // canvas.drawRect(GetDipsFromPixel(250),GetDipsFromPixel(200),GetDipsFromPixel(330),getBottom()-GetDipsFromPixel(480),mapPaint);
        ...
        canvas.drawPath(drawFloorNodes(107,116),navigationPaint);
    ...
    public Path drawFloorNodes(int start, int end){
        Path nav = new Path();
        //Start
        for(int i=0;i<roomEnterance.size();i++){
            if(roomEnterance.get(i).getId() == start){
                nav.moveTo(getLeft()+(getRight()/roomEnterance.get(i).getX()),GetDipsFromPixel(roomEnterance.get(i).getY()));
                nav.lineTo(getLeft()+(getRight()/2),GetDipsFromPixel(240));
            }
        }
        //Knoten Punkte
            getNodes(nav);
        //Ziel
        for(int i=0;i<roomEnterance.size();i++){
            if(roomEnterance.get(i).getId() == end){
    ...
    private void setCoordinates(){
        ...
        G114 = new Rect(0,GetDipsFromPixel(500),getLeft()+(int)(getRight()/2.5),getBottom()-GetDipsFromPixel(300));
